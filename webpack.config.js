...
var ipAddress=require("./util").getIPAddress();
...
var webpackDefaultConfig = {
    ...
    output: {
        ...
        publicPath:dev=="server"?"http://"+ipAddress+":"+ Config.port +"/":"",
...
//判断采用哪种模板
var templateUrl = (dev == "server"?"template/server.template.html":"template/template.json");
    templateUrl = (dev == "html" ? "template/server.template.html" : templateUrl);
//循环入口列表
chunkConfig.chunkList.forEach(function(item,i){

    webpackDefaultConfig.entry[item.chunk]=item.file+item.chunk;
    var filename = (dev =="server" ?item.chunk + ".html":item.fileName+".json");
        filename = (dev =="html" ?item.chunk + ".html":filename);
